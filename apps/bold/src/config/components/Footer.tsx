...
{navigation.map((item) => (<div key={item.key} className="pb-6">
<Link href={item.path} className="text-sm leading-6 text-gray-600 hover:text-gray-900">
{t(`header.navigation.${item.key}`)}
</Link>
</div>))}
</nav><p className="mt-10 text-center text-xs leading-5 text-gray-500">
{t("footer.copyright")}
</p>
</div>
</footer>)
