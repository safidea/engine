...
await fs.emptyDir('schema')

async function buildSchema(schema: string) {
  await exec(
    `bunx typescript-json-schema ./tsconfig.json ${capitalize(schema)}Schema --out ./schema/${schema}.schema.json --required --noExtraProps --strictNullChecks`
  )
